...
# manually correct one donor from TUM
# add individual_id
...
cam_pheno =  cam_pheno %>%
dplyr::select(shortID,Category,Age,Gender,Category_fine,Oligoclonal) %>%
...
# clean
pheno = pheno %>%
  dplyr::select(1,2,3,4,5,9,10,11) %>%
...
pheno_simple = all_combo@meta.data %>%
  ...
  dplyr::select(iid,Age,Sex,Category,ms_subtype,OCB,processing_site,source)
