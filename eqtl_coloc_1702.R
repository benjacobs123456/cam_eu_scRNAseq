...
eqtlgen = read_table("2019-12-11-cis-eQTLsFDR-ProbeLevel-CohortInfoRemoved-BonferroniAdded.txt.gz")
...
eqtlgen = eqtlgen %>% dplyr::select(SNP,SNPChr,FDR,GeneSymbol)
...
eqtls_filtered = eqtl_res %>%
  filter(chr_pos %in% eqtlgen_hg38_merged$chr_pos)
nrow(eqtls_filtered)

# join with eqtlgen FDR
eqtls_filtered  %>%
  left_join(eqtlgen_hg38 %>%
    dplyr::select(chr_pos,GeneSymbol) %>%
    dplyr::rename("gene" = GeneSymbol),
    by=c("chr_pos","gene"))
  mutate(sig_eqtl_gen = ifelse(,"eQTL","not eQTL"))
